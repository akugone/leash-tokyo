...
| `leash.maxSlippageBps` | text | optional, decimal basis points below 10000: widest price move a swap may allow. Empty: not enforced |
...
- `risk-manager`: granted with `grantSetterRoles(setText(name, "leash.dailyNotional", ""), riskManager)`, repeated for `leash.tokens` and `leash.maxSlippageBps`. The resolver decodes that calldata, derives `resource = keccak256(bytes(key))`, and grants `ROLE_SET_TEXT` scoped to that resource, for the whole resolver, not one name. The risk-manager cannot touch `addr`, any other key, or the registry.
