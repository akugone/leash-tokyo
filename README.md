...
5. Dashboard, **New agent** card: issue `trader-2` with its own cap, slippage and expiry, or issue `trader-1` again after the cut. Two owner transactions, no new contract. Agent terminal: `buy 20 lUSD of lETH as trader-2`, and the hook enforces the new mandate.
