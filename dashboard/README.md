...
`/api/demo/forbid` (risk-manager tries to revoke, must revert) `/api/demo/cut` (owner, `unregister`) and `/api/demo/issue` (owner, `register` a new or expired agent subname, then write its whole policy in one resolver `multicall`).
