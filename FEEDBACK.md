# Developer feedback: Uniswap v4 and ENSv2
...
1. **`BaseHook` moved out of v4-periphery.** The periphery README points to `Uniswap/v4-hooks-public`, whose repo drags in seventeen submodules (v3, v2, Pancake, UniswapX, Fluid...). We vendored the single `BaseHook.sol` file instead. A tiny `v4-hooks-base` package, or keeping `BaseHook` in periphery, would help hackathon teams.
2. **`optimizer_runs` trap under via-IR.** Our `beforeSwap` needs via-IR: without it, `LeashHook.sol` fails with stack too deep. With via-IR on, importing `Deployers` (and therefore `PoolManager`) into a test fails with a Yul stack too deep error inside `Pool.swap` at `optimizer_runs = 800`; 10000 and v4-core's own value (44444444) compile. The v4-template turns via-IR off, so this only bites hook authors who need it, and it is not documented anywhere they look first; a sentence in the "writing tests" page would save an hour.
